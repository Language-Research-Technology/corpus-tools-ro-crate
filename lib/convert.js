...
async function convertRoCrateToOcfl(options) {
    ...
    if(rocrate) {
      ...
      try {
        const rehydrated = new ROCrate(rocrate, { alwaysAsArray: true, resolveLinks: true });        
        corpus.crate = rehydrated;
        // rootDataset must track the injected crate's root so mintArcpId() updates the right entity
        corpus.rootDataset = rehydrated.root;
      } catch (e) {
