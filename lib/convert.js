...
async function convertRoCrateToOcfl(options) {
    ...
    const collectorOpts = {
        ...
        sf: runSiegfried,
        sfCachePath: siegfriedCachePath,
        inputs,
    ...
    let corpus;
    // This allows users to pass in an existing RO-Crate object (e.g. from ro-crate-js) 
    // instead of relying on Collector to read from disk, which is useful for programmatic use and testing. 
    // If options.rocrate is provided, it must be a valid RO-Crate object; otherwise, an error is thrown.
    if(rocrate) {
      if (rocrate === null || typeof rocrate !== 'object') {
        throw new TypeError('options.rocrate must be a ROCrate instance or a plain RO-Crate JSON object');
      }
      try {
        const rehydrated = new ROCrate(rocrate, { array: true, resolveLinks: true });       
        corpus = collector.newObject(collector.dataDir, rehydrated);
        corpus.crate = rehydrated; // Ensure the CollectorObject's crate is the rehydrated ROCrate instance
        corpus.rootDataset = corpus.crate.root; // Set the root dataset to the crate's root entity
      } catch (e) {
        throw new TypeError('options.rocrate must be a valid RO-Crate object: ' + e.message);
      }
    }else {
        corpus = collector.newObject(collector.dataDir);
    }
...
async function convertDistributed(collector, corpus, corpusRoot, corpusCrate, re) {
    const sourceDescriptor = corpusCrate.getEntity('ro-crate-metadata.json');
    ...
    for (const source of Array.from(externalized.values())) {
        ...
        copyCrateContext(corpusCrate, colObj.crate);
        ...
        // Preserve descriptor-level metadata (e.g., license) from the source crate
        // while keeping per-object structural descriptor fields intact.
        const targetDescriptor = colObj.crate.getEntity('ro-crate-metadata.json');
        if (sourceDescriptor && targetDescriptor) {
            for (const [key, value] of Object.entries(sourceDescriptor)) {
                if (key === '@id' || key === '@type' || key === 'about') continue;
                targetDescriptor[key] = JSON.parse(JSON.stringify(value));
            }
            colObj.crate.updateEntity('ro-crate-metadata.json', targetDescriptor);
        }
...
function copyCrateContext(sourceCrate, targetCrate) {
    const incoming = [].concat(sourceCrate.toJSON()["@context"] || []);
    const existing = new Set([].concat(targetCrate.context || []).filter(v => typeof v === "string"));

    for (const entry of incoming) {
        if (typeof entry === "string") {
            if (!existing.has(entry)) {
                targetCrate.addContext(entry);
                existing.add(entry);
            }
        } else {
            targetCrate.addContext(entry);
